...
\begin{document}
    ...
    \label{sec:basic}
        ...
        \begin{figure}
            \ExecuteMetaData[agda/tex/Report.lagda]{eval}
        ...
        The semantics of the bytecode is defined by a function (\texttt{exec})
        whose type can be read (because of \emph{currying}) as mapping a value of \texttt{Bytecode}
    ...
    \label{sec:conclusions}
        ...
        % Points where we got stuck (holes, postulates)
            % Basic correctness (BytecodeT), sequencing clause (proven assuming the lemma)
                 % Proving a lemma needed, lemma is being used OK

            % Fusion lemma for indexed folds, Theorem in the lifting module

            % What uses the coerceIdLemma1:
